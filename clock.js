...
var $mainslogan, $subslogan, subslogannum;
var progress, updateExam;
var now, updateTime;
// 正常或调试模式
...
else if (todate.match("2022-01-08|9")) change("高三理科");
// else if (todate.match("2022-01-15|6")) change("高二理科");
// 默认考试类型
else change("高三日常");
setInterval(updateSlogan, 2000);
// 考试标语轮播
function updateSlogan() {
  mainslogan = mainslogan || $mainslogan;
  subslogan = subslogan || $subslogan;
  subslogannum < subslogan.length - 1 ? subslogannum++ : subslogannum = 0;
  output("mainslogan", mainslogan);
  output("subslogan", subslogan[subslogannum]);
}
// “考试时钟的灵魂”
// 考试科目判断
function $(nextSubject, nextDate, nextStart, nextEnd, nextMainslogan, nextSubslogan) {
  if (now < end) console.log("当前科目未结束，故不注入科目: " + nextSubject);
    ...
    mainslogan = nextMainslogan || $mainslogan;
    subslogan = nextSubslogan || $subslogan;
    console.log(getClock(now) + "时成功注入科目: " + nextSubject + "\n开始时间: " + nextDate, nextStart + "\n结束时间: " + nextDate, nextEnd + ["\n指定大标语: ", "\n默认大标语: "][!nextMainslogan * 1] + mainslogan + ["\n指定副标语: ", "\n默认副标语: "][!nextSubslogan * 1] + subslogan);
  }
}
// 考试时钟科目时间更新
function updateExam() {
  ...
  if (now < (start - 6E5) && type.match("日常")) {
    ...
    // now.getHours() == 12 ?
    //   subslogan = ["12:05可能自动关机，请留意提示。"] : 0;
    // now.getHours() == 18 ?
    //   subslogan = ["警告: 考场周围应保持环境安静！"] : 0;
    ...
    updateSubject = function () {
      mainslogan = "考试结束";
      subslogan = ["式微式微，胡不归？", "考试人，考试魂，考试都是人上人",
